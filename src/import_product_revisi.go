...
func RunImportProductRevisiCmd(args []string) {
	...
	// ---- Sheet: Daftar Produk ----
	if err := importDaftarProdukRevisi(f, tx, *batchSize, *adminID, &messageDetailBuilder); err != nil {
		_ = tx.Rollback()
	...
	if err := importZatAktifProdukRevisi(f, tx, *batchSize, *adminID, &messageDetailBuilder); err != nil {
		_ = tx.Rollback()
	...
	// ---- Sheet: Grup Produk ----
	if err := importGrupProdukRevisi(f, tx, *batchSize, *adminID, &messageDetailBuilder); err != nil {
		_ = tx.Rollback()
	...
	// ---- Sheet: Izin Produk ----
	if err := importIzinProdukRevisi(f, tx, *batchSize, *adminID, &messageDetailBuilder); err != nil {
		_ = tx.Rollback()
...
func importDaftarProdukRevisi(f *excelize.File, tx *sql.Tx, batchSize int, adminID int, md *strings.Builder) error {
	sheet := "Daftar Produk"
	...
	for rows.Next() {
		...
		productIdPtr := getCol(0)
		if productIdPtr == nil {
			continue
		}
		productID := *productIdPtr

		productNamePtr := getCol(1)
		...
		expiredThreshold := "0"
		if p := getCol(17); p != nil {
			expiredThreshold = denormalizeNumber(p)
		}
		length := denormalizeNumber(getCol(18))
		lengthUnit := sql.NullInt64{Valid: false}
		if p := getCol(19); p != nil && *p != "" {
		...
		width := denormalizeNumber(getCol(20))
		widthUnit := sql.NullInt64{Valid: false}
		if p := getCol(21); p != nil && *p != "" {
			id, err := checkImportColumn(tx, "unit_name", "list_unit", *p, nil)
			if err != nil {
		...
		height := denormalizeNumber(getCol(22))
		heightUnit := sql.NullInt64{Valid: false}
		if p := getCol(23); p != nil && *p != "" {
			id, err := checkImportColumn(tx, "unit_name", "list_unit", *p, nil)
			if err != nil {
				return err
			}
		...
		weight := denormalizeNumber(getCol(24))
		weightUnit := sql.NullInt64{Valid: false}
		if p := getCol(25); p != nil && *p != "" {
			id, err := checkImportColumn(tx, "unit_name", "list_unit", *p, nil)
			if err != nil {
				return err
			}
		...
		volume := denormalizeNumber(getCol(26))
		volumeUnit := sql.NullInt64{Valid: false}
		if p := getCol(27); p != nil && *p != "" {
			id, err := checkImportColumn(tx, "unit_name", "list_unit", *p, nil)
			if err != nil {
				return err
			}
		...
		biggestConv := denormalizeNumber(getCol(28))
		biggestUnit := ""
		if p := getCol(29); p != nil {
			biggestUnit = *p
		}
		smallestConv := denormalizeNumber(getCol(30))
		smallestUnit := ""
		...
		manufacturer := ""
		if p := getCol(33); p != nil {
			manufacturer = *p
		}
		defaultMarginPrincipal := "0"
		if p := getCol(34); p != nil && *p != "" {
		...
		lockSale := 0
		if p := getCol(36); p != nil && strings.EqualFold(*p, "Ya") {
			lockSale = 1
		}
		stockLevelProduct := "0"
		...
		if p := getCol(41); p != nil && strings.EqualFold(*p, "Ya") {
		...
		defaultHna := "0"
		if p := getCol(43); p != nil && *p != "" {
			defaultHna = *p
		}
		productStatus := 1
		if p := getCol(44); p != nil && strings.EqualFold(*p, "Aktif") {
			productStatus = 2
		}
		...
		isConsignImport := 1
		if p := getCol(47); p != nil && strings.EqualFold(*p, "TIDAK") {
			isConsignImport = 0
		}
...
func importZatAktifProdukRevisi(f *excelize.File, tx *sql.Tx, batchSize int, adminID int, md *strings.Builder) error {
	sheet := "Zat Aktif Produk"
	rows, err := f.Rows(sheet)
	if err != nil {
		return fmt.Errorf("sheet '%s' not found: %w", sheet, err)
...
func importSupplierProdukRevisi(f *excelize.File, tx *sql.Tx, batchSize int, adminID int, md *strings.Builder) error {
	sheet := "Supplier Produk"
	rows, err := f.Rows(sheet)
	if err != nil {
		return fmt.Errorf("sheet '%s' not found: %w", sheet, err)
...
func importGrupProdukRevisi(f *excelize.File, tx *sql.Tx, batchSize int, adminID int, md *strings.Builder) error {
	sheet := "Grup Produk"
	rows, err := f.Rows(sheet)
	if err != nil {
		return fmt.Errorf("sheet '%s' not found: %w", sheet, err)
...
func importIzinProdukRevisi(f *excelize.File, tx *sql.Tx, batchSize int, adminID int, md *strings.Builder) error {
	sheet := "Izin Produk"
	rows, err := f.Rows(sheet)
	if err != nil {
		return fmt.Errorf("sheet '%s' not found: %w", sheet, err)
